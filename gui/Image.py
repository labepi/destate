...
class Image:
    ...
    def get_pixbuf(self, icon, image_type='png', force=False):
        """
        """
        if self.__path == None:
            return False

        if icon + image_type not in self.__cache.keys() or force:

            file = self.get_icon(icon, image_type)
            index = icon + image_type
            self.__cache[index] = gtk.gdk.pixbuf_new_from_file(file)
